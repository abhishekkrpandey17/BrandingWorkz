  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-white/30 backdrop-blur-md shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-3 flex items-center justify-between">
        ...
        <Link
          href="/"
          className="text-2xl font-bold text-indigo-600 transition-transform duration-900 transform hover:rotate-y-[360deg]"
        >
        ...
        <nav className="hidden md:flex items-center space-x-8">
          {navLinks.map((link) => (
            ...
            >
              {link.name}
              <span className="absolute left-0 -bottom-1 w-0 h-0.5 bg-indigo-600 group-hover:w-full transition-all duration-300" />
            </Link>
          ))}
